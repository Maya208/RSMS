...
function filterTable2(term, table, placeholder1, placeholder2) {
	dehighlight(table);
	var terms = term.value.toLowerCase().split(" ");
	var total = 0;
	var overall = 0;
	for (var r = 1; r < table.rows.length; r++) {
		...
		for (var i = 0; i < terms.length; i++) {
			if (table.rows[r].innerHTML.replace(/<[^>]+>/g, "").toLowerCase()
				...
				overall = overall + parseInt(table.rows[r].cells[8].innerHTML);
			}
			table.rows[r].style.display = display;
		}
	}
	if(typeof placeholder1 !== 'undefined')
		placeholder1.innerHTML = total;
	if(typeof placeholder2 !== 'undefined')
		placeholder2.innerHTML = overall;
}
/*
 * Transform back each
